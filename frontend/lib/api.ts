// API calls now go through Next.js proxy on same origin locally
// In production (Vercel), it can hit the Render backend directly via NEXT_PUBLIC_API_URL
const getApiBaseUrl = () => {
  // If explicitly set in environment (e.g., Vercel), use it. otherwise use relative for proxy
...
class ApiClient {
  ...
  // Products
  async getProducts(params?: { search?: string; page?: number; limit?: number; famille?: string; choix?: string; calibre?: string; sortBy?: string; sortOrder?: 'ASC' | 'DESC'; ids?: string }) {
    const query = new URLSearchParams(params as any).toString();
    return this.request(`/products?${query}`);
  }

  async getProductFilters() {
    return this.request<{ familles: string[]; brands: { brandid: number; brandname: string }[]; choix: string[] }>('/products/filters');
  }

  async getProductStats() {
    return this.request<{ totalqty: number; totalpallets: number; totalcolis: number; totalpurchasevalue: number; totalsalevalue: number; totalproducts: number }>('/products/stats');
  }
  ...
  async adjustProductQuantity(data: { productId: number; newTotalQty: number; qteparcolis?: number; qtecolisparpalette?: number; notes?: string }) {
    return this.request('/products/adjust-quantity', { method: 'POST', body: JSON.stringify(data) });
  }
  ...
  async getProductAdjustmentHistory(productId: number, params?: { startDate?: string; endDate?: string }) {
    const queryString = params ? new URLSearchParams(params as any).toString() : '';
  ...
  async getInventoryLevels(params?: {
    ...
    // Clean params - remove empty/falsy values and 'all' stock level
    const cleanParams: Record<string, string> = {};
    if (params) {
      Object.entries(params).forEach(([key, value]) => {
        // Skip undefined, null, empty strings, and 'all' stockLevel
        if (value !== undefined && value !== null && value !== '') {
          if (key === 'stockLevel' && value === 'all') return;
  ...
  async getAuditLogs(page: number = 1) {
    return this.request<{ data: any[]; total: number; page: number; limit: number }>(`/settings/audit?page=${page}`);
  }
  ...
  async getCustomerSituation(id: number, params?: { startDate?: string; endDate?: string }) {
    const query = params ? '?' + new URLSearchParams(params as any).toString() : '';
    return this.request<SituationResponse>(`/customers/${id}/situation${query}`);
  }
  ...
  // Orders
  async getOrders(params?: { status?: string; customerId?: number; search?: string; salesPersonId?: number; orderType?: string }) {
    // Clean params - remove empty/falsy values
    const cleanParams: Record<string, string> = {};
    if (params) {
      Object.entries(params).forEach(([key, value]) => {
        if (value !== undefined && value !== null && value !== '') {
  ...
  async updateOrderFinancials(orderId: number, data: {
    deliveryCost?: number;
    discount?: number;
    timber?: number;
    notes?: string;
    ...
    return this.request(`/orders/${orderId}/financials`, {
      method: 'PUT',
      body: JSON.stringify(data),
    });
  }
  ...
  async getPurchaseOrders(params?: { status?: string; factoryId?: number; brandId?: number; page?: number; limit?: number }) {
    const query = new URLSearchParams(params as any).toString();
  ...
  // --- Purchase Returns ---
  async getPurchaseReturns(params?: { factoryId?: number; status?: string; startDate?: string; endDate?: string }) {
    const query = new URLSearchParams(params as any).toString();
    return this.request(`/purchase-returns?${query}`);
  }

  async getPurchaseReturnById(id: number) {
    return this.request(`/purchase-returns/${id}`);
  }

  async createPurchaseReturn(data: any) {
    return this.request('/purchase-returns', {
      method: 'POST',
      body: JSON.stringify(data)
    });
  }

  async updatePurchaseReturnStatus(id: number, status: string) {
    return this.request(`/purchase-returns/${id}/status`, {
      method: 'PUT',
      body: JSON.stringify({ status })
    });
  }

  async deletePurchaseReturn(id: number) {
    return this.request(`/purchase-returns/${id}`, {
      method: 'DELETE'
    });
  }
  ...
  // --- Client Versements (Payments) ---
  async getClientVersements(params?: {
    accountId?: number;
    startDate?: string;
    endDate?: string;
    search?: string;
    ...
    const query = params ? '?' + new URLSearchParams(params as any).toString() : '';
    return this.request<any>(`/accounting/client-versements${query}`);
  }

  // --- Supplier Versements (Payments to Brands/Factories) ---
  async getSupplierVersements(params?: {
    accountId?: number;
    startDate?: string;
    endDate?: string;
    search?: string;
    supplierId?: number;
    supplierType?: 'BRAND' | 'FACTORY';
    limit?: number;
    offset?: number;
  }) {
    const query = params ? '?' + new URLSearchParams(params as any).toString() : '';
    return this.request<any>(`/accounting/supplier-versements${query}`);
  }

  async updateCashTransaction(id: number, data: {
    accountId?: number;
    amount?: number;
    tiers?: string;
    motif?: string;
