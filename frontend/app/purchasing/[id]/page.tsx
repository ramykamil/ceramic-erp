...
const getStatusBadge = (status: string): string => {
  ...
  return statusClasses[status as keyof typeof statusClasses] || 'bg-slate-800/50 text-slate-100';
};
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6 flex flex-wrap justify-between items-center gap-4">
          ...
          <div className="flex gap-2">
            ...
            <Link href="/purchasing" className="bg-slate-900/60 border border-white/\[0.08\] text-slate-200 px-4 py-2 rounded-lg hover:bg-slate-900/40 text-sm font-medium">
              ← Retour
            </Link>
          </div>
        </div>
        ...
        <div className="glassy-container p-6 mb-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-4 text-sm">
            ...
            <div>
              ...
              <p className="text-slate-100 font-semibold">{po.factoryname}</p>
            </div>
            <div>
              <label className="block text-slate-500 font-medium">Entrepôt de Livraison</label>
              <p className="text-slate-100 font-semibold">{po.warehousename}</p>
            </div>
            <div>
              <label className="block text-slate-500 font-medium">Type de Stock</label>
              <p className="text-slate-100 font-semibold">{po.ownershiptype === 'OWNED' ? 'Propre' : 'Consignation'}</p>
            </div>
            <div>
              <label className="block text-slate-500 font-medium">Date de Commande</label>
              <p className="text-slate-100 font-semibold">{formatDate(po.orderdate)}</p>
            </div>
            <div>
              <label className="block text-slate-500 font-medium">Date de Livraison Attendue</label>
              <p className="text-slate-100 font-semibold">{formatDate(po.expecteddeliverydate)}</p>
            </div>
            {po.notes && (
              <div className="col-span-2 md:col-span-4">
                <label className="block text-slate-500 font-medium">Notes</label>
                <p className="text-slate-100 bg-slate-900/40 p-2 rounded-md whitespace-pre-wrap">{po.notes}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="glassy-container overflow-hidden">
          <h2 className="text-xl font-semibold text-slate-200 p-5 border-b border-white/\[0.06\]">Articles Commandés</h2>
            ...
            <table className="w-full text-sm text-left text-slate-400">
              ...
              <tbody className="divide-y divide-white/\[0.06\]">
                {po.items.map((item) => {
                  ...
                  return (
                    <tr key={item.poitemid} className="hover:bg-sky-500/10/50">
              ...
              <tfoot className="bg-slate-900/40 border-t-2 border-white/\[0.08\]">
                <tr>
                  <td colSpan={6} className="px-6 py-3 text-right font-bold text-slate-200 text-base">
                    Total
                  </td>
