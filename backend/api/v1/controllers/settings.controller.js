    ...
    try {
        ...
        const query = `
            UPDATE AppSettings SET
                ...
                RetailMarginType = COALESCE($26, RetailMarginType),
                WholesaleMarginType = COALESCE($27, WholesaleMarginType),
                UpdatedAt = NOW(),
                UpdatedBy = $25
...
// BACKUP Database (Simple simulation)
async function triggerBackup(req, res, next) {
    try {
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
        console.log(`[BACKUP] Database backup requested at ${timestamp} by user ${req.user?.userId}`);

        res.json({
            success: true,
            message: `Sauvegarde créée avec succès: backup_${timestamp}.sql (Simulation)`,
...
async function updateUser(req, res, next) {
    try {
        const { id } = req.params;
        const { role, email, isactive, password, permissions } = req.body;
        ...
        if (password) {
            ...
            const hashedPassword = await bcrypt.hash(password, 10);
            query = `
                UPDATE Users SET Role=$1, Email=$2, IsActive=$3, PasswordHash=$4, Permissions=$5, UpdatedAt=NOW()
                WHERE UserID=$6
                RETURNING UserID, Username, Role, Email, IsActive, Permissions
            `;
            params = [role, email, isactive, hashedPassword, permissionsJson, id];
        } else {
            // Update without password change
            query = `
                UPDATE Users SET Role=$1, Email=$2, IsActive=$3, Permissions=$4, UpdatedAt=NOW()
                WHERE UserID=$5
                RETURNING UserID, Username, Role, Email, IsActive, Permissions
            `;
            params = [role, email, isactive, permissionsJson, id];
        }
...
async function triggerBackup(req, res, next) {
    try {
        ...
        res.json({ success: true, message: "Sauvegarde effectuée avec succès (Simulation)" });
    } catch (error) {
