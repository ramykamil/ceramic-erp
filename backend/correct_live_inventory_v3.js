...
async function fixInventory() {
    ...
    try {
        ...
        if (productIds.length === 0 && lines.length > 0) {
            console.log("Sample line 5: " + JSON.stringify(lines[5]));
        }
        ...
        for (const productId of productIds) {
            ...
            const prodRes = await client.query("SELECT p.ProductName, p.Size, p.QteParColis, p.QteColisParPalette, i.QuantityOnHand FROM Products p JOIN Inventory i ON p.ProductID = i.ProductID AND i.OwnershipType = 'OWNED' AND i.WarehouseID = 1 WHERE p.ProductID = $1", [productId]);
